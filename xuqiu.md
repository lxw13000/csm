...
> 本版相对上一版的核心变化：用户端、客服端由原生 App 调整为 **H5 实现**；新增 **多业务系统（多租户）对接层**，由业务系统提供「根据 user_id 查询用户信息」等接口，客服系统不再自建 C 端用户账号体系。
...
* **开放可对接**：以标准接口对接多个业务系统，业务系统只需提供「身份换取」与「按 user_id 查用户信息」两类接口即可接入。
...
| 用户信息来源 | 系统内维护 | **业务系统接口提供**；首次缓存 + 按需实时查询 |
| 业务系统对接 | 单一/隐式 | **多租户**，PC 后台注册接入并按租户隔离 |
...
业务系统C 站点 ─┘            │
            ...
            └───────────────┬───────────────┘
                            │ 出站调用(按app_id路由)
                            ▼
          业务系统对外接口：① 身份换取  ② 按user_id查用户信息
```

### 2.2 多租户与业务系统接入
* 每个业务系统在 PC 后台由**平台超级管理员**注册为一个**租户**，分配 `app_id` 与 `app_secret`，并配置其对外接口地址；同时为该租户开通至少一个**租户管理员账号**。
* 客服系统内 **C 端用户唯一标识 = `app_id` + `user_id`**（不同业务系统的 user_id 互不冲突）。
* 系统内统一以 **`app_id`**（业务系统/租户标识）作为所有业务数据（配置、用户、客服、QA、工单、会话、统计）的隔离键（详见 2.6）。
* 所有出站接口调用（身份换取、查用户信息）按 `app_id` 路由到对应业务系统，请求需带签名（基于 `app_secret`）。

**租户接入配置项**：`app_id`、`app_secret`、身份换取接口地址、用户信息查询接口地址、IP 白名单（可选）、启用状态。

### 2.3 用户身份获取流程（内嵌 WebView + token 换取）
1. 用户在业务 App 内点击「联系客服」。
2. 业务 App 向**自身后端**申请一个代表当前登录用户的**一次性临时 token**。
3. 业务 App 用 WebView 打开客服 H5，URL 携带 `app_id` 与 `token`，例如：
   `https://kf.example.com/h5?app_id=biz_a&token=xxxxx`
4. 客服 H5 将 `app_id + token` 提交客服系统后端。
5. 客服后端按 `app_id` 匹配租户配置，**调用该业务系统的「身份换取」接口**，用 token 换取 `user_id`。
6. 校验通过后以 `app_id + user_id` 确定唯一用户；**若为首次接入**，调用「按 user_id 查用户信息」接口缓存基础信息。
7. 客服后端下发**自身的会话凭证（session token）**给 H5，H5 据此建立 WebSocket、收发消息。

> 该方案下临时 token 不暴露真实 user_id、有效期短、一次性，安全性优于 URL 直接携带 user_id。

### 2.4 业务系统需提供的接口（对接方实现）
| 接口 | 入参 | 出参 | 用途 |
| --- | --- | --- | --- |
| ① 身份换取 | `app_id`、一次性 `token`、签名 | `user_id`（+ 可选基础信息） | WebView 进入时换取用户身份 |
| ② 按 user_id 查用户信息 | `app_id`、`user_id`、签名 | 用户详情（昵称、头像、等级、脱敏手机号、注册时间等） | 客服查看用户详情、首次接入缓存 |

> 若接口 ① 已直接返回足够的基础信息，可与接口 ② 合并；但建议保留接口 ② 以支持客服侧的按需实时查询。

### 2.5 用户信息处理策略（首次缓存 + 按需实时查询）
* **首次接入**：调用接口 ②，缓存昵称、头像等**展示用基础信息**到客服系统，用于会话列表/聊天界面快速展示。
* **按需实时**：客服查看「用户详情」时，实时调用接口 ② 拉取最新信息，避免缓存陈旧。
* 缓存仅作展示加速，**业务系统为用户信息的唯一权威来源**；接口不可用时，降级展示缓存信息并提示「信息可能非最新」。
...
│   ├── 身份接入
│   │   ├── 业务App传入 app_id + 临时token
│   │   ├── token换取 user_id（免独立登录）
│   │   └── 建立会话凭证与WebSocket
│   ├── 智能问答
│   │   ├── QA库优先匹配
│   │   ├── 最优解会话输出
...
### 4.1 用户端（H5）
1. **身份接入**：H5 内嵌于业务 App 的 WebView，进入时由业务 App 传入 `app_id + 临时token`，客服后端换取 `user_id` 并建立会话，用户**无需在客服系统单独注册登录**（详见 2.3）。
...
3. **会话展示**：层级显示「用户 → 工单 → 会话消息」，消息以实时聊天形式（WebSocket）加载；会话列表展示**未读消息数**、聊天界面展示消息**已读状态**（详见 4.5 已读回执）；用户信息默认展示缓存基础信息，点击「用户详情」时实时查询业务系统接口（详见 2.5）。
...
#### 4.3.1 平台超级管理员（跨租户）
1. **业务系统接入（多租户）**：注册业务系统、分配 `app_id/app_secret`、配置对接接口地址、启用/停用接入（详见 2.2）。
...
2. **C 端用户查看（只读）**：按 `app_id + user_id` 检索**本租户**用户，展示缓存信息并支持实时查询详情；C 端用户不在客服系统内创建/编辑。
...
* **会话连续性**：H5 刷新/重连后凭会话凭证恢复当前工单与消息上下文；token 过期需静默续期或重新换取。
...
* **已读回执**：客户端读取消息后经 `read` 通道上报已读到的最大序号；服务端按「工单 + 阅读方」记录已读水位（`last_read_seq`）并下发对端，用于渲染「已读」标记与计算未读数；采用高水位单行更新，不逐条记录（落库见 `csm_message_read`）。
* **降级策略**：WebSocket 不可用（代理拦截、网络限制等）时，自动降级为 **SSE / 长轮询接收下行 + HTTP POST 发送上行** 的组合；此为兜底通道，恢复后自动切回 WebSocket。
* **鉴权与安全**：连接握手时校验会话凭证（session token），过期触发静默续期或重新换取 token（详见 2.3）；服务端按 `app_id + user_id` / 客服账号校验消息收发权限，防止越权。
...
```mermaid
flowchart TD
    A[用户在业务App点击联系客服] --> B[业务App向自身后端申请一次性token]
    B --> C[WebView打开客服H5: 携带 app_id + token]
    C --> D[H5将 app_id + token 提交客服后端]
    D --> E{按 app_id 匹配租户配置}
    E -- 未命中/无效 --> X[拒绝接入]
    E -- 命中 --> F[调用业务系统'身份换取'接口]
    F --> G{token有效?}
    G -- 否 --> X
    G -- 是 --> H[获得 user_id]
    H --> I{该 app_id+user_id 是否首次接入?}
    I -- 是 --> J[调用'按user_id查用户信息'接口并缓存]
    I -- 否 --> K[复用已缓存用户]
    J --> L[下发客服系统会话凭证]
    K --> L
    L --> M[建立WebSocket, 进入智能问答]
```
...
```mermaid
flowchart TD
    A[H5 获得会话凭证] --> B[尝试建立 WebSocket]
    B --> C{握手鉴权通过?}
    C -- 否 --> D{token 过期?}
    D -- 是 --> E[静默续期/重新换取token] --> B
    D -- 否 --> X[提示接入失败]
...
| 实体 | 关键标识 / 字段 | 说明 |
| --- | --- | --- |
| 业务系统（租户） | `app_id`、`app_secret`、接口地址 | PC 后台注册，**作为全量数据隔离键**（详见 2.6） |
| C 端用户 | **`app_id` + `user_id`**（联合唯一） | 身份来自业务系统，信息按需查询/缓存；天然按租户隔离 |
...
| 会话消息 | 消息 ID、**`app_id`**、工单 ID、发送方、时间、类型 | 文本/图片等；用于耗时计算与记录 |
...
> 说明：① 客服系统不存储 C 端用户的权威资料，仅按 `app_id + user_id` 缓存展示信息，用户详情以业务系统接口为准；② **除平台超管外，所有实体的读写都强制限定在自身 `app_id` 范围内**，租户间数据互不可见（详见 2.6）。
